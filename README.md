...
`apps/web/app/content/gallery/` に画像を入れるだけで、`/gallery` ページにピンタレスト風に表示されます。
...
- `sharp` で **webp に変換**し、横幅を最大 **1920px** に調整します（拡大はしません）。
- ファイル名を**追加した順に連番（`001.webp`, `002.webp` …）へリネーム**します。
- 既に圧縮・連番済みの画像はスキップするため、繰り返し実行しても安全です。
- 実行には Node.js が必要です（`bun run` が内部で node を呼びます）。
